...
/* Capabilities flags known to medusaminer */
#define CAP_TEMP (1<<0)
...
static bool drillbit_getinfo(struct cgpu_info *drillbit, struct drillbit_info *info)
{
	...
	if (identity.protocol_version > MAX_VERSION) {
		drvlog(LOG_ERR, "Device firmware uses newer Drillbit protocol %d. We only support up to %d. Find a newer medusaminer!", identity.protocol_version, MAX_VERSION);
		return false;
	}
