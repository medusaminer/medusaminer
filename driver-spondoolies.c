...
static bool spondoolies_queue_full(struct cgpu_info *cgpu)
{
	...
	if (a->my_jobs[next_job_id].medusaminer_work) {
		ret = true;
		goto return_unlock;
	}
	...
	a->my_jobs[a->current_job_id].medusaminer_work = work;
...
static int64_t spond_scanhash(struct thr_info *thr)
{
	...
	if (a->parse_resp) {
		...
		for (i = 0; i < array_size; i++) { // walk the jobs
			...
			if ((a->my_jobs[job_id].medusaminer_work)) {
				if (a->my_jobs[job_id].merkle_root == work->mrkle_root) {
					...
					for (j = 0; j < 2; j++) {
						if (work->winner_nonce[j]) {
							...
							struct work *cg_work = a->my_jobs[job_id].medusaminer_work;
#ifndef SP_NTIME
					...
					if ((--a->my_jobs[job_id].ntime_clones) == 0) {
						//printf("Done with %d\n", job_id);
						work_completed(a->cgpu, a->my_jobs[job_id].medusaminer_work);
						a->good++;
						a->my_jobs[job_id].medusaminer_work = NULL;
				...
				a->empty++;
				printf("No medusaminer job (id:%d res:%d)!\n",job_id, work->res);
			}
		}
		mutex_unlock(&a->lock);
