			 ...
			 __maybe_unused char *data, size_t size, void *userdata)
{
	...
	switch(type) {
		...
		case CURLINFO_SSL_DATA_IN:
			pool->medusaminer_pool_stats.net_bytes_received += size;
			break;
		case CURLINFO_HEADER_OUT:
		case CURLINFO_DATA_OUT:
		case CURLINFO_SSL_DATA_OUT:
			pool->medusaminer_pool_stats.net_bytes_sent += size;
			break;
		case CURLINFO_TEXT:
		default:
			break;
		      ...
		      struct pool *pool, bool share)
{
	...
	pool->medusaminer_pool_stats.times_sent++;
	if (curl_easy_getinfo(curl, CURLINFO_SIZE_UPLOAD, &byte_count) == CURLE_OK)
		pool->medusaminer_pool_stats.bytes_sent += byte_count;
	pool->medusaminer_pool_stats.times_received++;
	if (curl_easy_getinfo(curl, CURLINFO_SIZE_DOWNLOAD, &byte_count) == CURLE_OK)
		pool->medusaminer_pool_stats.bytes_received += byte_count;
	...
	*rolltime = hi.rolltime;
	pool->medusaminer_pool_stats.rolltime = hi.rolltime;
	pool->medusaminer_pool_stats.hadrolltime = hi.hadrolltime;
	pool->medusaminer_pool_stats.canroll = hi.canroll;
	pool->medusaminer_pool_stats.hadexpire = hi.hadexpire;
...
/* These are medusaminer specific sleep functions that use an absolute nanosecond
...
/* This is a medusaminer gettimeofday wrapper. Since we always call gettimeofday
...
retry:
	...
	pool->medusaminer_pool_stats.times_sent++;
	pool->medusaminer_pool_stats.bytes_sent += ssent;
	pool->medusaminer_pool_stats.net_bytes_sent += ssent;
	return SEND_OK;
}
...
char *recv_line(struct pool *pool)
{
	...
	pool->medusaminer_pool_stats.times_received++;
	pool->medusaminer_pool_stats.bytes_received += len;
	pool->medusaminer_pool_stats.net_bytes_received += len;
out:
	if (!sret)
		clear_sock(pool);
...
/* medusaminer specific wrappers for true unnamed semaphore usage on platforms
