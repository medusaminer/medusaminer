...
static bool medusaminer_usb_lock_bd(struct device_drv *drv, uint8_t bus_number, uint8_t device_address)
{
...
static bool medusaminer_usb_lock(struct device_drv *drv, libusb_device *dev)
{
	return medusaminer_usb_lock_bd(drv, libusb_get_bus_number(dev), libusb_get_device_address(dev));
}

static void medusaminer_usb_unlock_bd(struct device_drv *drv, uint8_t bus_number, uint8_t device_address)
{
	struct resource_work *res_work;
...
static void medusaminer_usb_unlock(struct device_drv *drv, libusb_device *dev)
{
	medusaminer_usb_unlock_bd(drv, libusb_get_bus_number(dev), libusb_get_device_address(dev));
}
...
static void release_cgpu(struct cgpu_info *cgpu)
{
	if (__release_cgpu(cgpu))
		medusaminer_usb_unlock_bd(cgpu->drv, cgpu->usbinfo.bus_number, cgpu->usbinfo.device_address);
}
		  ...
		  bool single)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (found != NULL) {
			...
			if (is_in_use(list[i]) || medusaminer_usb_lock(drv, list[i]) == false)
				free(found);
			else {
				cgpu = device_detect(list[i], found);
				if (!cgpu)
					medusaminer_usb_unlock(drv, list[i]);
				else {
					new_dev = true;
	...
	if (cgpu->usbdev) {
		if (cgpu->usbdev->usb_type != USB_TYPE_FTDI) {
			applog(LOG_ERR, "%s: cgid %d latency request on non-FTDI device",
				cgpu->drv->name, cgpu->medusaminer_id);
...
fail:
	...
	snprintf(name, sizeof(name), "/tmp/medusaminer-usb-%d-%d", (int)bus_number, (int)device_address);
