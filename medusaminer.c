/*
 * Copyright 2014 James Sano
 * Copyright 2014 Alex Barkley
 * Copyright 2014 Michael Bubble
 *
...
static const char def_conf[] = "medusaminer.conf";
...
static struct opt_table opt_config_table[] = {
	...
	OPT_WITH_ARG("--api-description",
		     ...
		     "Description placed in the API status header, default: medusaminer version"),
	...
	OPT_WITH_ARG("--benchfile",
			...
			"Run medusaminer in benchmark mode using a work file - produces no shares"),
	OPT_WITHOUT_ARG("--benchfile-display",
			opt_set_bool, &opt_benchfile_display,
			"Display each benchfile nonce found"),
	OPT_WITHOUT_ARG("--benchmark",
			opt_set_bool, &opt_benchmark,
			"Run medusaminer in benchmark mode - produces no shares"),
#if defined(USE_BITFORCE)
...
{
	struct pool *pool = work->pool;
	struct medusaminer_pool_stats *pool_stats = &(pool->medusaminer_pool_stats);
...
static void calc_diff(struct work *work, double known)
{
	struct medusaminer_pool_stats *pool_stats = &(work->pool->medusaminer_pool_stats);
	double difficulty;
	uint64_t uintdiff;
...
static bool stale_work(struct work *work, bool share)
{
	 ...
	 * the nearest second */
	getwork_delay = pool->medusaminer_pool_stats.getwork_wait_rolling * 5 + 1;
...
retry:
	...
	wlogprint("[B]lacklist current device from current instance of medusaminer\n");
...
static void hash_sole_work(struct thr_info *mythr)
{
	...
	struct medusaminer_stats *dev_stats = &(cgpu->medusaminer_stats);
	struct medusaminer_stats *pool_stats;
	...
	while (likely(!cgpu->shutdown)) {
		...
		do {
			...
			pool_stats = &(work->pool->medusaminer_stats);
...
static int medusaminer_id_count = 0;
...
void enable_device(struct cgpu_info *cgpu)
{
	...
	devices[cgpu->medusaminer_id = medusaminer_id_count++] = cgpu;
...
static void hotplug_process(void)
{
	...
	for (i = 0; i < new_devices; i++) {
		...
		cgpu = devices[dev_no];
		enable_device(cgpu);
		cgpu->medusaminer_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
...
int main(int argc, char *argv[])
{
	...
	strcpy(opt_kernel_path, medusaminer_PREFIX);
	medusaminer_path = alloca(PATH_MAX);
	s = strdup(argv[0]);
	strcpy(medusaminer_path, dirname(s));
	free(s);
	strcat(medusaminer_path, "/");
	...
	if (cnfbuf) {
		...
		switch (fileconf_load) {
			...
			case -1:
				...
				if (use_curses)
					applog(LOG_WARNING, "Start medusaminer with -T to see what failed to load.");
				break;
			default:
				break;
		}
	...
	for (i = 0; i < total_devices; ++i)
		devices[i]->medusaminer_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
	...
	for (i = 0; i < total_pools; i++) {
		...
		pool->medusaminer_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
		pool->medusaminer_pool_stats.getwork_wait_min.tv_sec = MIN_SEC_UNSET;
	...
	while (!pools_active) {
		if (!pool_msg) {
			...
			pool_msg = true;
			if (use_curses)
				applog(LOG_ERR, "Press any key to exit, or medusaminer will wait indefinitely for an alive pool.");
		}
		if (!use_curses)
