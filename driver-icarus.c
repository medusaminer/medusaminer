...
static void transfer_read(struct cgpu_info *icarus, uint8_t request_type, uint8_t bRequest, uint16_t wValue, uint16_t wIndex, char *buf, int bufsiz, int *amount, enum usb_cmds cmd)
{
	...
	applog(LOG_DEBUG, "%s: cgid %d %s got err %d",
			icarus->drv->name, icarus->medusaminer_id,
			usb_cmdname(cmd), err);
}
*/
	...
	switch (ident) {
		...
		default:
			quit(1, "icarus_intialise() called with invalid %s cgid %i ident=%d",
				icarus->drv->name, icarus->medusaminer_id, ident);
	}
}
...
#if ROCKMINER_PRINT_DEBUG
	applog(LOG_WARNING, "thr_info id:%d, device_thread:%d, icarus medusaminer_id: %d, device_id: %d.", thr->id, thr->device_thread, icarus->medusaminer_id, icarus->device_id);
