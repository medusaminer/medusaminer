...
struct CODES {
 ...
 { SEVERITY_SUCC,  MSG_VERSION,	PARAM_NONE,	"medusaminer versions" },
 ...
 { SEVERITY_SUCC,  MSG_MINECONFIG,PARAM_NONE,	"medusaminer config" },
 ...
 { SEVERITY_SUCC,  MSG_MINESTATS,PARAM_NONE,	"medusaminer stats" },
 ...
 { SEVERITY_SUCC,  MSG_MINECOIN,PARAM_NONE,	"medusaminer coin" },
...
static void apiversion(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	root = api_add_string(root, "medusaminer", VERSION, false);
...
static void poolpriority(struct io_data *io_data, __maybe_unused SOCKETTYPE c, char *param, bool isjson, __maybe_unused char group)
{
	...
	// TODO: all medusaminer code needs a mutex added everywhere for change
...
static int itemstats(struct io_data *io_data, int i, char *id, struct medusaminer_stats *stats, struct medusaminer_pool_stats *pool_stats, struct api_data *extra, struct cgpu_info *cgpu, bool isjson)
{
	struct api_data *root = NULL;
...
static void minerstats(struct io_data *io_data, __maybe_unused SOCKETTYPE c, __maybe_unused char *param, bool isjson, __maybe_unused char group)
{
	...
	for (j = 0; j < total_devices; j++) {
		...
		if (cgpu && cgpu->drv) {
			...
			i = itemstats(io_data, i, id, &(cgpu->medusaminer_stats), NULL, extra, cgpu, isjson);
		}
	}
	...
	for (j = 0; j < total_pools; j++) {
		...
		sprintf(id, "POOL%d", j);
		i = itemstats(io_data, i, id, &(pool->medusaminer_stats), &(pool->medusaminer_pool_stats), NULL, NULL, isjson);
	}
...
static void *quit_thread(__maybe_unused void *userdata)
{
	...
	if (opt_debug)
		applog(LOG_DEBUG, "API: killing medusaminer");
...
static void *restart_thread(__maybe_unused void *userdata)
{
	...
	if (opt_debug)
		applog(LOG_DEBUG, "API: restarting medusaminer");
...
static void mcast()
{
	...
	char expect[] = "medusaminer-"; // first 8 bytes constant
	char *expect_code;
